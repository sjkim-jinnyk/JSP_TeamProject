...
public class RoomNumberDAO {
	...
	private static RoomNumberDAO instance = null;

	// 외부에서 객체 생성을 하지 못하게 private으로 접근을 제어
	private RoomNumberDAO() {}

	// 기본 생성자 대신에 싱긑턴 객체를 return을 해 주는 getInstance() 메서드를 만들어서 여기에 접근하게 하는 방법
	public static RoomNumberDAO getInstance() {
		if (instance == null) {
			instance = new RoomNumberDAO();
		}
		return instance;
	}
	...
	public String getRoomNumber(String name) {
		...
		try {
			...
			sql = "select * from room_number where room_name = ? order by room_number";
